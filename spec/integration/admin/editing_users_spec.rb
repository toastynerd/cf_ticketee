...
feature 'editing a user' do
  ...
  scenario "Updating a users's details" do
  ...
  scenario "Toggling a users's admin ability" do
    check "Is an admin?"
    click_button "Update User"
    page.should have_content("User has been updated.")
    within("#users") do
  ...
  scenario  "updating a user with an invalid e-mail fails" do
    fill_in "Email", :with => "fakefakefakefake"
    click_button "Update User"
